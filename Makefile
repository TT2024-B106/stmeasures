...
all: libeuclidean.so libmanhattan.so libeditdist.so liblcss.so libfrechet.so libhausdorff.so libdtw.so librdp.so libamss.so libsad.so
...
libamss.so:
	$(CC) $(CFLAGS) $(LDFLAGS) -o $@ src/amss.c
