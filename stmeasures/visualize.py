...
def geojsonio_contents(**kwargs) -> str:
    """
    Generates a GeoJSON string for visualization with geojson.io from input trajectory data.

    :param kwargs: Keyword arguments specifying trajectory data. Must include one of the following:

        - ``trajectories`` (list): A list of trajectories, where each trajectory is a list of 
          coordinates (each coordinate is a 2-element list of floats: ``[latitude, longitude]``).
        ...
        Optional arguments:
        - ``indexes`` (list): A list of integers representing metadata for trajectories. If provided, 
          it must match the length of ``trajectories``.

    :type kwargs: dict
    :return: A GeoJSON string formatted as a FeatureCollection containing the trajectories 
             or an exception if no valid argument is provided.
    :rtype: str

    :raises ValueError: If no arguments are provided, if the arguments do not match the 
                        expected format, or if ``indexes`` does not match the length of ``trajectories``.

    **Examples**

    .. code-block:: python

        # Example with multiple trajectories
        geojsonio_contents(
            trajectories=[
                [[19.603914, -99.01801], [19.60482, -99.016198]]
            ],
            indexes=[0]
        )
        # Output:
        ...
        #       "geometry": {
        #         "type": "LineString",
        #         "coordinates": [[-99.01801, 19.603914], [-99.016198, 19.60482]]
        #       },
        #       "properties": {
        #         "index": -1,
        #         "stroke": "#62a0ea",
        ...
        # Example with a single trajectory
        geojsonio_contents(
    ...
    .. note::
        This function expects coordinates to be in the format ``[latitude, longitude]`` 
        for ``trajectories`` or ``(latitude, longitude)`` for ``trajectory``. The 
        :func:`_swap_coordinates` helper function is used to adjust coordinates to the required 
        ``[longitude, latitude]`` order for GeoJSON output.
    """
    if not kwargs:
